...
module.exports = function cosplayer(dispatch) {
	...
	dispatch.hook('C_REQUEST_ACCESSORY_COST_INFO', 1, () => { // When wearing a custom outfit, allow free hat restyling
		if(mypreset && mypreset.gameId != 0) {
	...
	dispatch.hook('C_COMMIT_ACCESSORY_TRANSFORM', 1, event => { // When wearing a custom outfit, allow free hat restyling
		if(mypreset && mypreset.gameId != 0) {
			restyleHat(event)
			return false
		}
